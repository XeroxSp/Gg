...
local Items_Page = PepsiUi:CreateTab({
    Name = "Automatic"
})
local Raid = PepsiUi:CreateTab({
    Name = "Vitsure"
})
...
TestTab:AddToggle({
    Name = "Auto Farm",
	Value = false, -- ปรับค่าToggle true/false or Config
    Callback = function(t)
        _G.AutoFarm = t
...
local BossName = Boss:AddDropdown({
	Name = "Select Boss",
	Value = ...,
	List = BossList,
        ...
        if _G.FarmBoss then
            pcall(function()
                    ...
                    if game:GetService("ReplicatedStorage"):FindFirstChild(_G.SelectBoss) then
                        TP(game:GetService("ReplicatedStorage"):FindFirstChild(_G.SelectBoss).HumanoidRootPart.CFrame * CFrame.new(0,20,3))
                    end
                end     
            end)
        end
...
spawn(function()
    pcall(function()
        while task.wait() do
            if _G.AutoCoat and Old_World then
                TP(CFrame.new(-5099.94287, 154.810593, 4410.22314, -0.614755988, 1.02650006e-07, 0.788717389, 2.32714936e-09, 1, -1.2833415e-07, -0.788717389, -7.70587221e-08, -0.614755988))
                for i,v in pairs(game.workspace.Enemies:GetChildren()) do
                if v.Name == "Vice Admiral" then
                    repeat wait()
						   ...
						   game:GetService("VirtualUser"):CaptureController()
						   game:GetService("VirtualUser"):Button1Down(Vector2.new(1280, 670),workspace.CurrentCamera.CFrame)
                        until _G.AutoCoat == false or v.Humanoid.Health <= 0
                    end
                end 
            end
        end
...
BF:AddToggle({
	Name = "Bring All Fruit 50% Kick System",
